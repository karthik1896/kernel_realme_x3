...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->lcd_vci_gpio)) {
		...
		if (rc) {
			pr_debug("request for vci_gpio failed, rc=%d\n", rc);
	...
	if (gpio_is_valid(r_config->err_flag_gpio)) {
		...
		if (rc) {
			pr_debug("request for err_flag_gpio failed, rc=%d\n", rc);
...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->reset_config.lcd_vci_gpio)) {
		...
		if (rc)
			pr_debug("unable to set lcd_vci_gpio dir for disp gpio rc=%d\n", rc);
	}
	#endif /* OPLUS_BUG_STABILITY */
	...
	if(!strcmp(panel->name, "samsung sofef03f_m amoled fhd+ panel with DSC")){
		if (gpio_is_valid(panel->reset_config.lcd_mode_sel_gpio)) {
				...
				if (rc) {
					pr_debug("unable to set dir for lcd_mode_sel_gpio gpio rc=%d\n", rc);
				}
		}
...
/*Chen.Pan@BSP.TP.Function, 2020/10/23, Add for tp use noflash tp*/
	if(true != is_pd_with_guesture) {
		pr_debug("[TP]:power_on-is_pd_with_guesture = false\n");
...
*/
	...
	if(panel->oppo_priv.is_19696_lcd) {
		if(esd_check || (0 == tp_gesture_enable_flag())){
			...
			TPS65132_power_enable(panel,false);
		#endif /* OPLUS_FEATURE_TPS65132 */
			pr_debug("[LCD] ESD check to TPS65132 power down\n");
		}
	}
	#endif /* OPLUS_BUG_STABILITY */
...
/*xupengcheng@MULTIMEDIA.DISPLAY.LCD, 2020/12/29, add for samsung 90fps Global HBM backlight issue*/
	if (panel->is_hbm_enabled && bl_lvl != 0) {
		if(power_change_update_backlight == 1) {
			...
			pr_debug("dsi_cmd global hbm power changed\n");
		}
		return 0;
	}
#else
...
static int dsi_panel_parse_reset_sequence(struct dsi_panel *panel)
{
	...
	pr_debug("oppo,reset_timing: %s", panel->reset_timing ? "true" : "false");
	#endif /* OPLUS_BUG_STABILITY */
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	{
		pr_debug("failed to read oplus,lcd-delay-disp-en-gpio\n");
	...
	if (utils->read_u32(utils->data, "oplus,lcd-delay-reset-gpio",
		&panel->reset_config.lcd_delay_reset_gpio))
	{
		pr_debug("failed to read oplus,lcd-delay-reset-gpio\n");
	...
	{
		pr_debug("failed to read oplus,lcd-delay-mode-sel-gpio\n");
	...
	{
		pr_debug("failed to read oplus,lcd-delay-set-pinctrl-state\n");
	...
	{
		pr_debug("failed to read oplus,lcd-delay-enable-regulator\n");
	...
	{
		pr_debug("failed to read oplus,lcd-delay-vci-gpio\n");
	...
	{
		pr_debug("failed to read oplus,lcd-delay-lp11-state\n");
...
/*Sachin Shukla@PSW.MM.Display.LCD.Stable,2019-10-24 add for fingerprint */
		...
		if (rc)
			pr_debug("failed to parse oppo config, rc=%d\n", rc);
#endif
...
/*Chen.Pan@BSP.TP.Function, 2020/10/23, Add for tp use noflash tp*/
	...
	if(!strcmp(panel->name, "samsung sofef03f_m amoled fhd+ panel with DSC")){
		...
		if (rc)
			pr_debug("unable to set lcd_delay_lp11_state dir for disp gpio rc=%d\n", rc);
	}
	...
	/*Chen.Pan@BSP.TP.Function, 2020/10/23, Add for tp use noflash tp*/
	mode = get_boot_mode();
	pr_debug("[TP] in dsi_panel_power_on, mode=%d\n",mode);
...
int dsi_panel_unprepare(struct dsi_panel *panel)
{
	...
	if(panel->oppo_priv.is_19696_lcd){
		if(esd_check || (0 == tp_gesture_enable_flag())){
			if (gpio_is_valid(panel->reset_config.reset_gpio)){
				gpio_set_value(panel->reset_config.reset_gpio, 0);
				pr_debug("[LCD] ESD check to reset_gpio 0\n");
			}
		}else{
			if (gpio_is_valid(panel->reset_config.reset_gpio)){
				gpio_set_value(panel->reset_config.reset_gpio, 1);
				pr_debug("[LCD] ESD check to reset_gpio 1\n");
			}
		}
	}
	#endif /*OPLUS_BUG_STABILITY */
